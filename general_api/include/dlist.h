...
/*two  double direction lists manage, include idle and ready double direction lists*/
typedef struct queuemng {
   struct list_head   list_idle, list_ready;
}queuemng_t; 

/*init the list manage, each list has one element, and the pre and next ptr point to the element itself*/
#define QUEUEMNG_INIT(name) \
	queuemng_t name = { LIST_HEAD_INIT(name.list_idle), LIST_HEAD_INIT(name.list_ready) }
